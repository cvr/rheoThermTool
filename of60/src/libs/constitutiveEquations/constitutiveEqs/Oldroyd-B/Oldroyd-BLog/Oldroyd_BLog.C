...
Foam::tmp<Foam::volScalarField> Foam::constitutiveEqs::Oldroyd_BLog::dotTHfun()
{
...
void Foam::constitutiveEqs::Oldroyd_BLog::correct()
{
    ...
    (
	  ...
	  + (eigVecs_ & (inv(eigVals_) - Itensor) & eigVecs_.T()) / lambda_
    ...
    tau_ = etaP() / lambda_ * symm((eigVecs_ & eigVals_ & eigVecs_.T()) - Itensor);
    tau_.correctBoundaryConditions();
}

Foam::tmp<Foam::volScalarField> Foam::constitutiveEqs::Oldroyd_BLog::energyExtraTerms()
{
    return tmp<volScalarField>
    (
        new volScalarField
        (
            ...
            (
              ...
              + (1.0 - elastEnergDiss_) * tr(tau_)/(2.0*lambda_)
            )
        )
    );
}
